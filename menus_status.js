...
const MenuStatus = {
    ...
    renderGuild: (content) => {
        ...
        const rows = acceptedIds.map(id => {
            ...
            const canStartDungeon = def.kind === 'guildDungeon' && !ready;
    ...
    startGuildQuestChallenge: (questId) => {
        if (typeof Guild === 'undefined' || typeof Guild.startChallengeQuest !== 'function') return false;
        if (typeof Dungeon !== 'undefined' && Dungeon.isGuildQuestRunActive?.()) {
