...
if (!wallGraphicBlock.includes('Field.getDungeonWallFaceThemeForDraw()')) errors.push('dungeon wall faces do not resolve through the shared theme registry');
if (!wallGraphicBlock.includes('wallTile?.lowerLayer === true') || !wallGraphicBlock.includes('wallTile?.animatedWater === true')) errors.push('water-surface W tiles can be replaced by wall faces');
if (!wallGraphicBlock.includes('Field.currentMapData.wallFaceTorchImg || theme?.accentImg')) errors.push('theme-specific accent wall face is not selected');
if (!wallGraphicBlock.includes("Field.getRenderedTileForDraw(tileX, tileY + 1, mapW, mapH, areaKey) === 'W'")) errors.push('wall faces are no longer restricted to exposed W rows');
