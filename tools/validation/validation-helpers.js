...
globalThis.WORLD_MAPS = WORLD_MAPS;
globalThis.ABYSS_REGION_MASTER = ABYSS_REGION_MASTER;
...
function isEffectAt(effect, x, y) {
  ...
  if (Array.isArray(effect.excludePoints) && effect.excludePoints.some(point =>
    Number(point?.x) === tx && Number(point?.y) === ty
  )) return false;
...
function findTileEffect(mapDef, x, y) {
  return (mapDef.tileEffects || []).find(effect => {
    if (!isEffectAt(effect, x, y)) return false;
    const tile = getTile(mapDef, x, y);
    if (tile === 'W') return false;
    if (effect.type !== 'ice' && effect.type !== 'poison') return true;
    if (['C', 'R', 'B', 'S', 'D', 'U', 'X', 'Y', 'Z', 'Q', 'N', 'O'].includes(tile)) return false;
