...
assert(chainActions.some(action => action.type === 'BOSS' && Number(action.value) === 302100 && action.winEventId === 'abyss_azelgarag_clear'), 'ヴェグナシス勝利後にアゼルガラグ戦へ連続移行する');
...
const pillars = bossMonsters.filter(monster => Number(monster.id) >= 302080 && Number(monster.id) <= 302084);
assert(pillars.length === 5 && pillars.every((monster, index) => Number(monster.imageId) === 302080 && monster.linkedBattleGroup === 'vegnasis' && Number(monster.linkedDeathIndex) === index && Number(monster.gutsLevel) >= 10), 'ヴェグナシスは1画像・5攻撃対象・高根性の連結ボスとして正本化されている');
assert(Number(bossMonsters.find(monster => Number(monster.id) === 302100)?.imageId) === 302100 && Number(bossMonsters.find(monster => Number(monster.id) === 302101)?.imageId) === 302101, 'アゼルガラグ第一・第二形態は別の差し替え用画像IDを持つ');
