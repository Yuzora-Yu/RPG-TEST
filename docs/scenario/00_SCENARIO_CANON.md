...
3. `docs/development-policy.md` と矛盾しない長期方針
4. 現在の `story.js` / `map.js` に実装されている内容
5. Codexが追加した草稿

注意:

- 現在の実装は重要な資料だが、自動的に最終正史ではない。
...
- `water_temple_clear` で魔法の小舟 `108` を得る。
...
### Notes
- 水上都市クリア時の解放内容は、長期方針と現行実装に差がある。
- 長期方針: 水上都市で `boat` + `casino`、雷の要塞で `medalKing`。
