...
- 始まりの村の長老イベントでガイル `109` とサラ `110` が加入し、`storyStep: 1 / subStep: 1` へ進む。
- 最初の依頼会話ではOPを再生しない。始まりの洞穴攻略後、長老への報告 `PROLOGUE3` が終了して `storyStep: 2 / subStep: 1` へ進んだ直後、`openingPrologue3Viewed` 未設定時だけ紙芝居OPを再生する。全画像取得の確認は初回起動時に済ませ、OP後には重複表示しない。
- OP開始時は一度完全暗転し、短い静止時間とフェード解除を挟んでから本文を自動進行する。本文には変更可能な主人公名、始まりの村の固有名、村内で起きた出来事の逐語的説明を出さない。プリズマ崩壊、滅びた村、火の里までの異変、不穏な気配、旅の動機に情報を限定し、最後に `main.html` と同じタイトルロゴを表示する。
- 初回起動時に全画像データを今ダウンロードするか確認する。「はい」は完了を待って開始し、「いいえ」は即開始して全画像をバックグラウンドでキャッシュする。どちらでもリュミナ村、開幕ジェリー戦、始まりの洞穴のマップチップ・通常敵・ボス・フィールド／洞窟戦闘背景をメモリ先読みし、OP以前の戦闘画像欠落を防ぐ。
- リュミナ村の3人の住民は、会話回数をセーブして台詞を循環表示する。村マップの洞穴入口には人物を追加しない。最初の洞穴内部に既存配置されている見張りイベントへ村人画像を設定し、人物の一マス手前から会話する。
- 洞穴の見張りが進行条件を満たして消える場合、会話完了時点でアクション判定とPhaser静的層を同時に更新する。マップ遷移や遅延画像取得でも同じ再構築経路を使い、一歩移動するまで旧表示を残さない。
- フィールド画面へ切り替える際は、表示領域の寸法を保ったまま不可視状態で `Field.init()`、Phaser描画有効化、静的層再構築を完了し、同じ呼び出し内の最後にだけ可視化する。最初のワールドマップ表示に必要な海・床・森・高山・低山・村オーバーレイは起動時必須キャッシュに含める。
- 宿屋の「転送の扉」は、ランダム深淵が解放されるまでボタン自体を生成しない。`???` の予告表示も行わない。`abyssFirstEntered` は初回進入履歴としてのみ保持し、表示・利用条件は `abyssRandomUnlocked` と `unlocked.teleport` に一本化する。旧保存データもロード時に同条件へ同期する。
...
- どちらの勝利ルートでも、この場ではレイラを加入させず、`lightPalaceCleared` を立てて `storyStep: 8 / subStep: 0` へ進む。
- 地下牢の看守 `301071` を倒して牢を開き、レイラを調べる。宮殿クリア前は呪いで会話不能、クリア後は容体が安定する。
- 世界樹の葉（item `5`）を所持している場合、渡すか選択できる。渡すと1個消費し、`light_palace_prison_leila` 内でレイラ `204` が加入して `leilaJoined` を立てる。
- レイラ未加入の間はガルヴァニア洞窟北口・魔王城方面の導線を封鎖し、地下牢へ戻る案内を出す。
...
- 深淵世界編の最終ボスタイルは、死幻の魔柱ヴェグナシスと深淵王アゼルガラグが共有する。
- 初回は `abyss_final_altar_encounter` からヴェグナシス5柱戦へ入る。第1戦勝利時は固定ボスマスを討伐済みにせず、`abyssVegnasisDefeated` を立ててアゼルガラグ戦へ連続移行する。
- アゼルガラグ戦で全滅した場合、終焉の祭壇へ戻ると同じボスマスにアゼルガラグ `302100` が表示され、ヴェグナシスを再戦せず最終戦から再開できる。
- アゼルガラグ討伐時だけ `abyssAzelgaragDefeated` を立ててボスマスを消し、リュシオンから光の翼（item `109`）を授かり、`wing` を解放する。光の翼はメダル景品に含めない。
- エンドロール後、終焉の祭壇の亀裂を調べる `abyss_postgame_crack` で `abyssRandomUnlocked`、`abyssDungeonMenuUnlocked`、`dungeonMenu`、`teleport` を同時に解放し、ランダム深淵へ入る。これ以降、宿屋に転送の扉が表示される。

### 10. 移動・施設の現行正本

- メダル王はストーリークリア条件を持たず、到達できれば利用できる施設タイル導線とする。
- スカイプリズム（item `110`）は通常の道具屋商品に含まれる。店頭流通は意図した現行仕様であり、物語専用報酬にはしない。
- 光の翼（item `109`）だけは終焉の祭壇でアゼルガラグを倒した後、リュシオンから授かる固有報酬とする。
...
- レイラ `204`: 光の宮殿クリア後、地下牢で世界樹の葉を渡すと加入。初期Lv40。
...
- `tools/validation/validate-key-door-runtime.js` は、現行の複数鍵報酬、固定ダンジョン鍵タイル、固定鍵スコープに合わせて更新済み。
...
- ワールドマップ座標 `x:9, y:59` に、独立30階ダンジョン「追憶の魔境」を追加。解放フラグは設けず、座標へ到達できれば入場できる。
- 本編深淵とは進行判定を共有しない。入場中も `abyssFirstEntered`、左上の本編目的、深淵挑戦回数、深淵世界の帰還先を変更しない。
- スカイプリズムの登録順は「最果ての祠」の直前。初回到達時に発見済みとして登録する。
- 挑戦開始階は常に1階。1階をRank91相当とし、1階進むごとに敵・装備・通常アイテムの抽選基準をRank+1する。
- 通常階は地上ダンジョンの外観、戦闘背景、生息モンスターを同一モチーフから抽選する。Rank1～85の通常モンスターを元データとし、戦闘時にRank91～120相当へ再生成する。
- 指定ストーリーボスIDは通常敵枠にも低確率で出現する。通常敵枠では同Rank帯通常敵の基礎値へ正規化し、物語ボス本来のHP・経験値・Gold・固有ドロップを持ち込まない。
- 追憶の魔境内では、加入可能な撃破モンスター1体ごとに独立して1%の仲間化抽選を行う。指定ストーリーボスは通常階・30階ボスのどちらも仲間化対象。
- 10階と20階は休憩所。Rank100／110相当の道具屋・武器屋・防具屋、高額宿屋、鍛冶屋、錬金屋を置き、ランダム戦闘は発生しない。
- 30階は指定候補から選ぶRank120相当のボス戦。勝利後、Rank120相当レア装備宝箱3個、ランダム特性書宝箱1個、ランダムスキル書宝箱1個、回復の泉、地上帰還ゲートを出現させる。宝箱報酬は周回ごとに再取得でき、初回限定報酬は設けない。踏破実績は報酬なしで自動記録する。
- 海底神殿3階のMAP代表ボス画像はモンスターID `301030` に固定。
- スキル書の表示名と説明文は `skillId` を正本として `SKILLS_DATA` から動的生成し、将来のスキル名・説明変更へ追従する。
- 詳細仕様は `docs/MEMORY_REALM_SPEC_20260731.md` を正本とする。

### 2026-07-31確認範囲

- 今回はブラウザ上の通しプレイ検証を実施せず、入口・階層状態・敵生成・報酬・仲間化・施設・帰還処理のコード経路とデータ参照を静的に確認した。
- 既存の「検証結果と評価」に記載された2026-06-15時点の検証結果は、当時の履歴として維持する。
...
- `tools/validation/validate-map-safety.js`
- `tools/validation/validate-key-door-runtime.js`
