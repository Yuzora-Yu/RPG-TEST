...
const DB = {
    SKILLS: [
        ...
        // ★修正: レート上昇、守備大幅ダウン
        {id:413, name:'裁きの雷霆', type:'魔法', target:'ランダム', mp:45, rate:0.9, count:5, base:100, elm:'雷', buff:{def:0.5}, desc:'雷魔法超ダメージ5連・守備激減'},
        // ★修正: ベース999に強化
        ...
        // ★修正: レート上昇、全ステータスダウン
        {id:610, name:'シャイニングブレス', type:'魔法', target:'全体', mp:20, rate:1.4, count:1, base:150, elm:'光', buff:{atk:0.8, def:0.8, spd:0.8, mag:0.8}, desc:'光のブレス・全能力ダウン'},
        // ★修正: レート上昇、全属性耐性ダウン(-50)
        {id:611, name:'ダークネスブレス', type:'魔法', target:'全体', mp:20, rate:1.4, count:1, base:150, elm:'闇', buff:{elmResDown:50}, desc:'闇のブレス・属性耐性ダウン'},
    ...
    OPT_RULES: [
        ...
        {key:'elmRes', elm:'火', name:'火耐', unit:'%', allowed:['R','SR','SSR','UR','EX'], min:{R:5,SR:10,SSR:20,UR:40,EX:80}, max:{R:9,SR:19,SSR:39,UR:79,EX:150}},
        {key:'elmRes', elm:'水', name:'水耐', unit:'%', allowed:['R','SR','SSR','UR','EX'], min:{R:5,SR:10,SSR:20,UR:40,EX:80}, max:{R:9,SR:19,SSR:39,UR:79,EX:150}},
        {key:'elmRes', elm:'風', name:'風耐', unit:'%', allowed:['R','SR','SSR','UR','EX'], min:{R:5,SR:10,SSR:20,UR:40,EX:80}, max:{R:9,SR:19,SSR:39,UR:79,EX:150}},
        {key:'elmRes', elm:'雷', name:'雷耐', unit:'%', allowed:['R','SR','SSR','UR','EX'], min:{R:5,SR:10,SSR:20,UR:40,EX:80}, max:{R:9,SR:19,SSR:39,UR:79,EX:150}},
        {key:'elmRes', elm:'光', name:'光耐', unit:'%', allowed:['SSR','UR','EX'], min:{SSR:10,UR:20,EX:40}, max:{SSR:19,UR:39,EX:80}},
        {key:'elmRes', elm:'闇', name:'闇耐', unit:'%', allowed:['SSR','UR','EX'], min:{SSR:10,UR:20,EX:40}, max:{SSR:19,UR:39,EX:80}}
    ],
...
// ... (CONST, MAP_DATA, DB.SKILLS, DB.ITEMS, DB.CHARACTERS, DB.EQUIPS, DB.OPT_RULES ... までは変更なし) ...
// ※ DB.EQUIPS は空配列定義のままでOK（下の自動生成で入るため）
...
(() => {
    ...
    // ★修正: minRank (出現開始階層) を設定
    const MONSTER_TYPES = [
        { minRank:1,  name:'スライム', hp:300, atk:80, def:80, spd:80, mag:50, exp:10, gold:10 },
        { minRank:1,  name:'ドラキー', hp:250, atk:90, def:60, spd:130, mag:100, exp:12, gold:15 },
        { minRank:5,  name:'ゴースト', hp:350, atk:90, def:70, spd:110, mag:130, exp:20, gold:20 },
        { minRank:10, name:'さまようよろい', hp:400, atk:110, def:160, spd:50, mag:20, exp:25, gold:30 },
        { minRank:15, name:'ホイミスライム', hp:280, atk:60, def:70, spd:100, mag:120, exp:20, gold:20 }, // ベースとしても定義
        { minRank:20, name:'ベビーサタン', hp:250, atk:80, def:70, spd:110, mag:200, exp:50, gold:50 },
        { minRank:30, name:'オーク', hp:500, atk:130, def:90, spd:70, mag:40, exp:40, gold:35 },
        { minRank:40, name:'アークデーモン', hp:400, atk:120, def:100, spd:90, mag:180, exp:100, gold:90 },
        { minRank:50, name:'キラーマシン', hp:450, atk:150, def:140, spd:120, mag:30, exp:80, gold:80 },
        { minRank:60, name:'ドラゴン', hp:600, atk:170, def:110, spd:80, mag:80, exp:150, gold:120 },
        { minRank:80, name:'キラーマシン2', hp:750, atk:200, def:180, spd:130, mag:50, exp:250, gold:250, actCount:2 }
    ];

    // ★修正: 新スキルを組み込み、行動パターンを強化
    const MONSTER_SKILL_SETS = {
        'スライム': { 
            low: [1, 10, 2],                  // 攻撃, メラ, 防御
            mid: [1, 301, 40, 101],           // 攻撃, メラミ, 火炎斬り, 強撃
            high: [1, 305, 407, 601, 603, 605] // 攻撃, メラゾーマ, メラガイアー, 火炎の息, 激しい炎, しゃくねつ
        },
        'ドラキー': { 
            low: [1, 14, 2],                  // 攻撃, ドルマ, 防御
            mid: [1, 307, 61, 46],            // 攻撃, ドルモーア, ボミオス, ウイングアッパー
            high: [1, 307, 410, 607, 52, 611] // 攻撃, ドルモーア, シャイニング, 毒の息, ピオリム, ダークネスブレス
        },
        'さまようよろい': { 
        ...
        'ゴースト': { 
            ...
            high: [1, 305, 307, 47, 60, 611]  // 攻撃, メラゾーマ, ドルモーア, ブラッドソード, ルカニ, ダークネスブレス
        },
        'オーク': { 
        ...
        'キラーマシン': { 
            ...
            high: [1, 203, 401, 105, 302, 414] // 攻撃, さみだれ剣, ギガブレイク, しんくうは, ベギラマ, グランドクロス
        },
        'アークデーモン': { 
        ...
        'ドラゴン': { 
            ...
            high: [1, 605, 604, 609, 404, 107] // 攻撃, しゃくねつ, かがやく息, 煉獄火炎, メテオ, テールスイング
        },
        'ホイミスライム': { 
            low: [1, 20, 2],                  // 攻撃, ホイミ, 防御
            mid: [1, 21, 51, 12],             // 攻撃, ベホイミ, スカラ, バギ
            high: [1, 22, 23, 24, 309, 610]   // 攻撃, ベホマラー, ベホマ, ベホマズン, バギクロス, シャイニングブレス
        },
        'ベビーサタン': { 
            low: [1, 308, 10],                // 攻撃, イオラ, メラ
            mid: [1, 306, 602, 607],          // 攻撃, イオナズン, こごえる吹雪, 毒の息
            high: [1, 412, 606, 500, 307, 415] // 攻撃, イオグランデ, 絶対零度, マダンテ, ドルモーア, 絶対零度(魔法)
        },
        'キラーマシン2': { 
            low: [1, 41, 44],                 // 攻撃, はやぶさ, 兜割り
            mid: [1, 203, 608, 42],           // 攻撃, さみだれ剣, 稲妻, 氷結斬り
            high: [1, 401, 409, 608, 106, 413] // 攻撃, ギガブレイク, ギガクロス, 稲妻, 天下無双, 裁きの雷霆
        }
    };

    for(let r=1; r<=100; r++) {
        // ★修正: 現在の階層(r)で解放されているモンスターのみを候補にする
        const candidates = MONSTER_TYPES.filter(m => m.minRank <= r);
        
        // 候補からランダムに最大3種選ぶ (重複なし)
