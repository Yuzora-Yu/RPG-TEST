/* monsters.js - direct source balance data generated 2026-05-19 */
const MONSTER_RACES = {
  "antiDemon": [
    "死霊",
    "魔族"
...
const FIXED_RARE_MONSTERS = [
  {
    ...
    "traits": [],
    "archives": [
      "とても固く逃げ足が速いが、倒すと大きな経験を得られるとして人気の魔物。"
    ]
  },
  {
    "id": 200202,
...
function getFloorOffset(floor, band) {
  ...
  if (band.offsetType === 'low_1_5') return LOW_FLOOR_OFFSETS[1][index];
  if (band.offsetType === 'low_6_10') return LOW_FLOOR_OFFSETS[6][index];
...
function generateBandMonster(floor) {
  ...
  if (!band || !band.monsters || band.monsters.length === 0) return null;
...
function tryGenerateRareMonster(floor) {
  ...
  if (Math.random() >= getRareEncounterRate(floor)) return null;
...
function generateEnemyForFloor(floor, options = {}) {
  ...
  if (floor >= 1 && floor <= 200) return generateBandMonster(floor);
  return null;
}
